...
MerchantAPI.prototype.login = function (guid, options) {
  var successResponse = {
    guid: guid,
    success: true,
    message: warnings.LOGIN_DEPRECATED,
  };
  return this.getWallet(guid, options).then(function () {
...
MerchantAPI.prototype.getAddressBalance = function (guid, options) {
  return this.getWallet(guid, options).then(function (wallet) {
    ...
    return {
      ...
      total_received: addr.totalReceived,
    };
  });
};
  ...
  return this.getWallet(guid, options)
    ...
    .then(function (wallet) {
      ...
      function success(tx) {
        ...
        var message =
          tx.to.length > 1 ? "Sent to Multiple Recipients" : "Payment Sent";
        metrics.recordSend();
        return {
          to: tx.to,
          amounts: tx.amounts,
      ...
      function error(e) {
        ...
        if (msg === "NO_UNSPENT_OUTPUTS") {
          ...
          msg = {
            ...
            needed: satoshiToBTC(
              p.amounts.reduce(add, p.balance - p.sweepAmount)
            ),
  ...
  return this.getWallet(guid, options)
    ...
    .then(function (wallet) {
      return q.Promise(function (resolve, reject) {
        ...
        function success() {
          resolve({ address: a.address, label: a.label });
        }
      });
    })
    .catch(function (e) {
...
MerchantAPI.prototype.upgradeWallet = function (guid, options) {
  return this.getWallet(guid, options)
    ...
    .then(function (wallet) {
      ...
      var hdwallet = wallet.upgradeToV3(
        options.label,
        options.second_password,
        success,
        error
      ...
      function success(s) {
        deferred.resolve(formatAcct(hdwallet.accounts[0]));
      }
...
MerchantAPI.prototype.listAccounts = function (guid, options) {
  return this.getWalletHD(guid, options).then(function (wallet) {
    if (options.account == null) {
      var activeAccounts = wallet.hdwallet.accounts.filter(
        byProp("active", true)
      );
...
function requireSecondPassword(options) {
  return function (wallet) {
    if (
      ...
      !wallet.validateSecondPassword(options.second_password)
    ) {
      throw "ERR_SECPASS";
    }
    return wallet;
